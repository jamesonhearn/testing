...
public class TETile {
    ...
    // Introduced render height/width
    ...
    public TETile(char character, Color textColor, Color backgroundColor, String description,
        ...
        this(character, textColor, backgroundColor, description, filepath, id, Double.NaN,
                Double.NaN); // start with placeholder values
    }
    ...
    public TETile(char character, Color textColor, Color backgroundColor, String description, int id) {
        this(character, textColor, backgroundColor, description, null, id, Double.NaN,
                Double.NaN);
    ...
    /**
     * Full constructor with explicit render dimensions. Width and height are relative to the size of
     * a single tile (1.0 = 16x16), allowing oversized sprites while keeping their base aligned to
     * the bottom of the tile cell.
     */
    public TETile(char character, Color textColor, Color backgroundColor, String description,
                  String filepath, int id, double renderWidth, double renderHeight) {
    ...
    public void draw(double x, double y, double width, double height) {
        if (filepath != null) {
                // neighboring cells when sprite dimensions exceed 16×16.
                try {
                    double centerX = x + 0.5;
                    if (Double.isNaN(width) || Double.isNaN(height)) {
                        StdDraw.picture(centerX, y + 0.5, filepath);
                    } else {
                        double centerY = y + height / 2.0;
                        StdDraw.picture(centerX, centerY, filepath, width, height);
                    }
                    return;
